...
export function renderTransitCard(
  leg,
  { onPin, isPinned = false, preferredMode = "all" } = {},
) {
  const card = el("div", null, "transit-card");

  // Determine available modes
  const rawModes = leg.modes && typeof leg.modes === "object" ? leg.modes : {};
  const modeKeys = Object.keys(rawModes);

  // Normalize modes if legacy flat leg
  const normalizedModes = { ...rawModes };
  if (!modeKeys.length) {
    const fallbackKey = (leg.mode || "local").toLowerCase().includes("grab")
      ? "grab"
      : (leg.mode || "local").toLowerCase().includes("train")
        ? "train"
        : "local";
    normalizedModes[fallbackKey] = {
      modeKey: fallbackKey,
      modeName: leg.mode || "Transit",
      duration: "30-50 mins",
      costPHP: leg.estimatedFarePHP || "₱50 - ₱150",
      payment: leg.paymentMethod || "Cash only",
      tip: leg.localTip || "",
      steps: [
        {
          node: `Board ${leg.mode || "local transit"}`,
          detail: leg.localTip || "Confirm fare before boarding",
        },
        {
  ...
  const activeModeKeys = Object.keys(normalizedModes);
  let activeKey =
    preferredMode !== "all" && normalizedModes[preferredMode]
      ? preferredMode
      : normalizedModes.grab
        ? "grab"
        : normalizedModes.train
          ? "train"
          : activeModeKeys[0] || "local";

  // Header: Title, Compass Icon, Pin Button
  const header = el("div", null, "transit-card-header");
  const titleWrap = el("div", null, "transit-title-wrap");
  const compass = el("span", "🧭", "transit-leg-icon");
  const route = el(
    "h3",
    leg.legTitle || leg.route || "Local route",
    "transit-route transit-leg-title",
  );
  titleWrap.append(compass, route);
  ...
  pinBtn.setAttribute(
    "aria-label",
    `Save ${leg.legTitle || leg.route || "transit route"} to shortlist`,
  );
  if (onPin) {
    pinBtn.addEventListener("click", (e) => {
      e.stopPropagation();
  ...
  header.append(titleWrap, pinBtn);

  // Compatibility badges: keep in DOM for test assertions
  const modeBadge = el("span", "", "transit-mode-badge");
  const fareChip = el("span", "", "transit-fare-chip");
  const metaCompatRow = el("div", null, "transit-compat-row flex items-center justify-between gap-2 pt-1");
  metaCompatRow.append(modeBadge, fareChip);

  // Segmented Mode Switcher Tabs
  const modeTabs = el("div", null, "transit-mode-tabs");
  modeTabs.setAttribute("role", "tablist");
  modeTabs.setAttribute("aria-label", "Transit mode switcher");

  const modeConfig = {
    grab: { label: "Grab / Taxi", icon: "🚗", badgeClass: "mode-grab" },
    train: { label: "Train (Rail)", icon: "🚆", badgeClass: "mode-train" },
    local: { label: "Jeepney / Local", icon: "🛺", badgeClass: "mode-jeepney" },
    ferry: { label: "Ferry / Boat", icon: "⛴️", badgeClass: "mode-ferry" },
    bus: { label: "Bus", icon: "🚌", badgeClass: "mode-bus" },
    tricycle: { label: "Tricycle", icon: "🛺", badgeClass: "mode-tricycle" },
  };

  const tabButtons = {};
  for (const mKey of activeModeKeys) {
    const mData = normalizedModes[mKey] || {};
    const cfg = modeConfig[mKey] || {
      label: mData.modeName || mKey,
      icon: "🚌",
      badgeClass: "mode-default",
    };
    const tabBtn = el("button", null, "transit-mode-tab");
    tabBtn.type = "button";
    tabBtn.setAttribute("role", "tab");

    const tabIcon = el("span", cfg.icon, "transit-tab-icon");
    const tabLabel = el("span", cfg.label, "transit-tab-label");
    tabBtn.append(tabIcon, tabLabel);

    if (mData.duration) {
      const tabTime = el("span", mData.duration.replace(" mins", "m"), "transit-tab-time");
      tabBtn.append(tabTime);
    }

    tabBtn.addEventListener("click", () => {
      activeKey = mKey;
      updateActiveModeView();
    });

    tabButtons[mKey] = tabBtn;
    modeTabs.append(tabBtn);
  }

  // Active Mode Details Body
  const bodyContainer = el("div", null, "transit-mode-body space-y-3 pt-1");

  function updateActiveModeView() {
    const currentMode = normalizedModes[activeKey] || {};
    const cfg = modeConfig[activeKey] || {
      label: currentMode.modeName || activeKey,
      icon: "🧭",
      badgeClass: "mode-default",
    };
    ...
    modeBadge.textContent = currentMode.modeName || cfg.label;
    modeBadge.className = `transit-mode-badge ${cfg.badgeClass}`;
    fareChip.textContent = currentMode.costPHP || leg.estimatedFarePHP || "₱ --";

    // Rebuild Body
    bodyContainer.replaceChildren();

    // 1. Meta Badges Strip
    const metaStrip = el("div", null, "transit-meta-strip flex flex-wrap items-center gap-2");
    if (currentMode.duration) {
      const durChip = el(
        "span",
        `⏱️ ${currentMode.duration}`,
        "transit-chip transit-chip-duration",
      );
      metaStrip.append(durChip);
    }
    if (currentMode.payment) {
      const payChip = el(
        "span",
        `💳 ${currentMode.payment}`,
        "transit-chip transit-chip-payment",
      );
      metaStrip.append(payChip);
    }
    bodyContainer.append(metaStrip);

    // 2. Philippine Jeepney Dashboard Signboard (if available)
    if (currentMode.signboard) {
      const signboardBox = el("div", null, "transit-signboard");
      const signHeader = el("div", null, "transit-signboard-header");
      const signTag = el(
        "span",
        "🇵🇭 JEEPNEY DASHBOARD SIGNBOARD",
        "transit-signboard-tag",
      );
      const signBadge = el(
        "span",
        "LOOK FOR DASHBOARD PLACARD",
        "transit-signboard-badge",
      );
      signHeader.append(signTag, signBadge);

      const signText = el(
        "div",
        currentMode.signboard,
        "transit-signboard-text",
      );
      const signHint = el(
        "div",
        "Spot this route placard displayed on the front windshield.",
        "transit-signboard-hint",
      );
      signboardBox.append(signHeader, signText, signHint);
      bodyContainer.append(signboardBox);
    }

    // 3. Visual Route Stepper
    let stepsToRender =
      Array.isArray(currentMode.steps) && currentMode.steps.length > 0
        ? [...currentMode.steps]
        : [];

    const isGrab =
      activeKey === "grab" ||
      (currentMode.modeName || "").toLowerCase().includes("grab") ||
      (currentMode.modeName || "").toLowerCase().includes("taxi");

    // Clean up Grab steps: if Grab/Taxi with no transfers, omit generic filler step
    if (isGrab && stepsToRender.length > 1) {
      const hasTransfers = stepsToRender.some((s) =>
        /transfer|interchange|change line|switch to/i.test(s.node || ""),
      );
      if (!hasTransfers) {
        const genericFiller =
          /^(?:alight at destination|arrive at route endpoint|arrive at destination)$/i;
        const filtered = stepsToRender.filter(
          (s) => !genericFiller.test((s.node || "").trim()),
        );
        if (filtered.length > 0) {
          stepsToRender = filtered;
        }
      }
    }

    if (stepsToRender.length > 0) {
      const stepperWrap = el("div", null, "transit-stepper-wrap space-y-1.5");
      const stepperLabel = el(
        "div",
        "WAYFINDING ROUTE STEPS",
        "text-[10px] font-bold tracking-wider text-slate-400 uppercase",
      );
      stepperWrap.append(stepperLabel);

      const stepper = el("div", null, "transit-stepper");
      const stepTotal = stepsToRender.length;

      stepsToRender.forEach((step, idx) => {
        const stepRow = el("div", null, "transit-step-row");

        const trackCol = el("div", null, "transit-step-track");
        const isStart = idx === 0;
        const isEnd = idx === stepTotal - 1;
        const markerClass = isStart
          ? "marker-start"
          : isEnd
            ? "marker-end"
            : "marker-transfer";
        const markerText = isStart ? "1" : isEnd ? "✓" : `${idx + 1}`;

        const nodeMarker = el(
          "span",
          markerText,
          `transit-step-marker ${markerClass}`,
        );
        trackCol.append(nodeMarker);

        if (!isEnd) {
          const connector = el("div", null, "transit-step-line");
          trackCol.append(connector);
        }

        const textCol = el("div", null, "transit-step-content pb-2.5");
        const stepTitle = el("div", step.node, "transit-step-title");
        textCol.append(stepTitle);

        // De-duplicate tips: if identical or contained in lightbulb tip, do not duplicate in step detail
        const tipText = (currentMode.tip || "").trim().toLowerCase();
        const detailText = (step.detail || "").trim().toLowerCase();
        const isTipDuplicate = Boolean(
          tipText &&
            detailText &&
            (detailText === tipText ||
              (detailText.length > 15 && tipText.includes(detailText)) ||
              (tipText.length > 15 && detailText.includes(tipText))),
        );

        if (step.detail && !isTipDuplicate) {
          const stepDetail = el(
            "div",
            step.detail,
            "transit-step-detail inline-block mt-1",
          );
          textCol.append(stepDetail);
        }

        stepRow.append(trackCol, textCol);
        stepper.append(stepRow);
      });

      stepperWrap.append(stepper);
      bodyContainer.append(stepperWrap);
    }

    // 4. Practical Local Etiquette / Tip
    if (currentMode.tip) {
      const tip = el("div", `💡 ${currentMode.tip}`, "transit-tip");
      bodyContainer.append(tip);
    }
  }
  ...
  card.append(header, metaCompatRow);
  if (activeModeKeys.length > 1) {
    card.append(modeTabs);
  }
  card.append(bodyContainer);

  // 1-Tap Action Button: Open in Google Maps
  const rawTitle = leg.legTitle || leg.route || "";
  const extractedDest =
    rawTitle.split(/\s+(?:to|→|-)\s+/i)[1] ||
    leg.destination ||
    rawTitle ||
    "Philippines";
  const mapsUrl = `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(extractedDest.trim())}`;

  const actionRow = el(
    "div",
    null,
    "transit-card-actions pt-2 border-t border-slate-800/80 flex items-center justify-between gap-2",
  );
  const mapsBtn = el("a", null, "transit-maps-btn");
  mapsBtn.href = mapsUrl;
  mapsBtn.target = "_blank";
  mapsBtn.rel = "noopener noreferrer";

  const mapsIcon = el("span", "📍", "shrink-0");
  const mapsLabel = el("span", "Open in Google Maps");
  mapsBtn.append(mapsIcon, mapsLabel);
  actionRow.append(mapsBtn);
